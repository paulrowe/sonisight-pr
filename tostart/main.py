...
# when Gemini returns "None" on an image it's usually a true negative (normal
# tissue). Falling back to saliency overrides that and manufactures false
# positives, so the fallback is off by default now that Gemini ROI works well.
# Flip to True to restore the old behavior (also useful for ablation runs).
USE_SALIENCY_FALLBACK = False
...
def get_roi_from_gemini(pil_img: Image.Image) -> list | None:
    ...
    prompt = (
        "You are assisting with breast ultrasound triage.\n"
        "A large fraction of these images are NORMAL breast tissue with NO discrete mass.\n"
        "Your job is FIRST to decide whether a discrete focal lesion is actually present, "
        "and only then to locate it.\n\n"
        "A discrete lesion is a bounded, focal region (typically hypoechoic or heterogeneous) "
        "that stands out from the surrounding tissue.\n"
        "Do NOT box normal fibroglandular tissue, diffuse shadowing, speckle, or imaging artifacts.\n\n"
        "If a discrete lesion is clearly present:\n"
        "  respond ONLY with its bounding box like: [x, y, w, h]\n"
        "If no discrete lesion is clearly present (normal tissue):\n"
        "  respond ONLY with: None\n\n"
        "When uncertain whether a region is a true lesion or just normal tissue, prefer None.\n"
        "No extra text. Coordinates refer to the pixel grid of the provided image."
    )
...
def segment_candidate(gray: np.ndarray, in_roi: bool = False):
    # in_roi=True means we're segmenting INSIDE a Gemini-localized box, where a
    # mass is known to be present and legitimately fills/touches the crop. In
    # that case we drop the whole-image gatekeeping (border, area-cap, solidity)
    # that would otherwise reject tightly-boxed and irregular (malignant) masses.
    ...
    def contour_score(c):
        ...
        # inside a tight ROI the mass SHOULD touch the crop edges - don't reject it
        if touches_border and not in_roi:
            return -1e9, None
        area_frac = area / A_img
        # whole image: ignore tiny/enormous blobs. inside an ROI the mass can fill
        # most of the crop, so allow a much larger upper bound.
        lo, hi = (0.02, 0.97) if in_roi else (0.005, 0.45)
        ...
        # spiculated/irregular (malignant) masses are low-solidity on purpose, so
        # relax the solidity floor inside an ROI.
        solidity_floor = 0.15 if in_roi else 0.30
        if ar > 3.5 or solidity < solidity_floor:
            return -1e9, None
        ...
        # score: prefer round, solid, dark, smooth. The edge-noise penalty is for
        # whole-image search (avoid picking speckle); inside an ROI it would push
        # us off a real spiculated mass, so drop it there.
        edge_penalty = 0.0 if in_roi else 0.8 * max(0.0, ring_edges - 0.25)
        score = (
            1.0 * circularity +
            1.2 * contrast_out_in +
            0.6 * solidity -
...
def extract_descriptors(bgr: np.ndarray, in_roi: bool = True) -> Dict:
    gray = preprocess(bgr)
    cnt, mask = segment_candidate(gray, in_roi=in_roi)
    ...
    # “no mass” gate: if it’s tiny & low contrast, don’t call it a mass.
    # The high-edge clause (ring_edges > 0.36) only applies in whole-image mode:
    # inside a Gemini ROI, high edge density usually IS spiculation, so dropping
    # the mass for it would discard the most malignant-looking lesions.
    too_noisy = (ring_edges > 0.36) and not in_roi
    if ((area_frac < 0.006 and contrast_out_in < 0.08) or too_noisy):
        return {
            "mass_present": False,
            "img_quality": "ok",
            "shape": "none",
...
async def predict(
    ...
    roi_source = "gemini" if box is not None else None

    # if Gemini can’t find anything, optionally fall back to local saliency.
    # Off by default: a Gemini "None" is usually a real normal image.
    if box is None and USE_SALIENCY_FALLBACK:
        box = local_roi_from_saliency(bgr)
        if box is not None:
            roi_source = "saliency-fallback"
                ...
                if not descriptors.get("mass_present", False):
                    ...
                    if cnt_s is not None and mask_s is not None:
                        ...
                        if desc2.get("mass_present", False):
                            descriptors = desc2
                            descriptors["roi_resegmented"] = True
                        else:
    ...
    # record where the ROI actually came from, so evals can audit gemini vs fallback
    descriptors["detection_method"] = roi_source if roi_source else "none"
